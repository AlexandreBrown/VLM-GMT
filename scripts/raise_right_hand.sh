#!/bin/bash
# Task: raise_right_hand (text-only, no scene objects)
# Robot raises its right hand above its head and keeps it there.
# Success: right hand Z > 1.3m at episode end.
#
...
# GT hand target: above head, slightly right
HAND_POS="0.1 -0.2 1.5"

# ── 1. Create empty scene (local) ────────────────────────────────────────────
python $VLMGMT/tasks/raise_right_hand/create_scene.py \
    --output $VLMGMT/outputs/raise_right_hand_scene.pt
...
python $VLMGMT/pipeline/generate_motion.py \
    --task raise_right_hand --condition baseline \
    --output-dir $VLMGMT/outputs/raise_right_hand/baseline \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT

# ── 3. Generate gt + vlm motions — CLUSTER ───────────────────────────────────
# GT
python $VLMGMT/pipeline/generate_motion.py \
    --task raise_right_hand --condition gt \
    --cube-world-pos $HAND_POS \
    --output-dir $VLMGMT/outputs/raise_right_hand/gt \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT

# VLM 32B (text-only, no --image)
python $VLMGMT/pipeline/generate_motion.py \
    --task raise_right_hand --condition vlm \
    --vlm-name qwen2.5-vl-32b \
    --output-dir $VLMGMT/outputs/raise_right_hand/vlm_32b \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT
...
python $VLMGMT/pipeline/generate_motion.py \
    ...
    --output-dir $VLMGMT/outputs/raise_right_hand/vlm_7b \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT
...
for COND in baseline gt vlm_7b vlm_32b; do
    python $VLMGMT/eval/run_eval.py \
        --checkpoint $CKPT \
        --motion-file $VLMGMT/outputs/raise_right_hand/${COND}/motion.pt \
        --scenes-file $VLMGMT/outputs/raise_right_hand_scene.pt \
        --task raise_right_hand --condition ${COND} \
        --num-episodes 50 --simulator isaaclab \
        --output-dir $VLMGMT/outputs/raise_right_hand/results \
        --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT
done
