#!/bin/bash
# Task: point_at_obj_with_left_hand
# Robot points at a blue object to its right with its left hand (reaches across).
# Success: dist(left_hand, obj) < 0.15m at episode end.
#
# Run order:
...
# ── 1. Create scene (local) ──────────────────────────────────────────────────
python $VLMGMT/tasks/point_at_obj_with_left_hand/create_scene.py \
    --obj-pos $OBJ_POS \
    --output $VLMGMT/outputs/point_at_obj_with_left_hand_scene.pt
...
python $VLMGMT/pipeline/generate_motion.py \
    --task point_at_obj_with_left_hand --condition baseline \
    --output-dir $VLMGMT/outputs/point_at_obj_with_left_hand/baseline \
...
python $VLMGMT/pipeline/capture_egocentric.py \
    ...
    --motion-file $VLMGMT/outputs/point_at_obj_with_left_hand/baseline/motion.pt \
    --robot-name g1 --simulator isaaclab --num-envs 1 \
    --scenes-file $VLMGMT/outputs/point_at_obj_with_left_hand_scene.pt \
    --output-dir $VLMGMT/outputs/point_at_obj_with_left_hand \
    --pitch-deg 50 --robot-yaw-deg 0 --prefix ego

# ── 4. Generate gt + vlm motions — CLUSTER ───────────────────────────────────
# GT
python $VLMGMT/pipeline/generate_motion.py \
    --task point_at_obj_with_left_hand --condition gt \
    --cube-world-pos $OBJ_POS \
    --output-dir $VLMGMT/outputs/point_at_obj_with_left_hand/gt \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT

# VLM 32B (scp ego.png to cluster first)
python $VLMGMT/pipeline/generate_motion.py \
    --task point_at_obj_with_left_hand --condition vlm \
    --image $VLMGMT/outputs/point_at_obj_with_left_hand/ego.png \
    --vlm-name qwen2.5-vl-32b --pitch-deg 50 \
    --output-dir $VLMGMT/outputs/point_at_obj_with_left_hand/vlm_32b \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT
...
python $VLMGMT/pipeline/generate_motion.py \
    ...
    --output-dir $VLMGMT/outputs/point_at_obj_with_left_hand/vlm_7b \
    --protomotions-root $PROTOMOTIONS --vlm-gmt-root $VLMGMT
