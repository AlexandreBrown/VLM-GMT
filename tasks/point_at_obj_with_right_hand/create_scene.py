"""Create a point_at_obj_with_right_hand scene.

A colored sphere (blue cube standing in for a sphere) placed to the right
and slightly forward of the robot at about chest height. No table: the
object is on a thin pedestal so it's clearly visible and reachable.

The object is placed to the RIGHT (-Y in IsaacLab) so that the baseline
(generic "point forward" motion) misses it.
...
Usage:
    python tasks/point_at_obj_with_right_hand/create_scene.py \
        --output outputs/point_at_obj_with_right_hand_scene.pt
"""
...
# Object at ~0.6m forward, 0.2m to the LEFT, chest height (~0.9m)
...
def create_scene(
    ...
    output: str = "outputs/point_at_obj_with_right_hand_scene.pt",
) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", default="outputs/point_at_obj_with_right_hand_scene.pt"
    )
